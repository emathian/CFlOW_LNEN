...
def export_hist(c, gts, scores, threshold):
    ...
    image_dirs = os.path.join(c.viz_dir, c.class_name,c.model)
    ...
    if not c.infer_train:
        ...
        image_file = os.path.join(image_dirs, 'hist_images_train_' + datetime.datetime.now().strftime("%Y-%m-%d-%H:%M:%S"))
...
def export_groundtruth(c, test_img, gts):
    os.makedirs(os.path.join(c.viz_dir, c.class_name), exist_ok= True)
    if not c.infer_train:
        image_dirs = os.path.join(c.viz_dir,  c.class_name,  c.model, 'gt_images_' + datetime.datetime.now().strftime("%Y-%m-%d-%H:%M:%S"))
    else:
        image_dirs = os.path.join(c.viz_dir,  c.class_name,  c.model, 'gt_images_train_' + datetime.datetime.now().strftime("%Y-%m-%d-%H:%M:%S"))
    # images
...
def export_scores(c, test_img, scores, threshold):
    ...
    image_dirs = (c.viz_dir, c.class_name, c.model, 'sc_images_' + datetime.datetime.now().strftime("%Y-%m-%d-%H:%M:%S"))
    if not c.infer_train:
        image_dirs = os.path.join(c.viz_dir,  c.class_name,  c.model, 'sc_images_' + datetime.datetime.now().strftime("%Y-%m-%d-%H:%M:%S"))
    else:
        image_dirs = os.path.join(c.viz_dir,  c.class_name,  c.model, 'sc_images_train_' + datetime.datetime.now().strftime("%Y-%m-%d-%H:%M:%S"))
    # images
    if not os.path.isdir(image_dirs):
...
def export_test_images(c, test_img, gts, scores, threshold):
    os.makedirs(os.path.join(c.viz_dir, c.class_name), exist_ok= True)
    if not c.infer_train:
        image_dirs = os.path.join(c.viz_dir,  c.class_name,  c.model, 'images_' + datetime.datetime.now().strftime("%Y-%m-%d-%H:%M:%S"))
    else:
        image_dirs = os.path.join(c.viz_dir,  c.class_name,  c.model, 'images_train_' + datetime.datetime.now().strftime("%Y-%m-%d-%H:%M:%S"))
    cm = 1/2.54
    # images
    if not os.path.isdir(image_dirs):
...
def export_results_df(c, res_df):
    os.makedirs(os.path.join(c.viz_dir, c.class_name), exist_ok= True)
    if not c.infer_train:
        res_tab_name = 'results_table.csv'
    else:
        res_tab_name = 'results_table_train.csv'
    res_df.to_csv(os.path.join(c.viz_dir, c.class_name, res_tab_name))

def write_anom_map(c, super_mask, files_path_list):
    os.makedirs(os.path.join(c.viz_dir, c.class_name), exist_ok= True)
   
    if not c.infer_train:
         anom_map_folder = 'anomaly_map'
    else:
        anom_map_folder = 'anomaly_map_train'
